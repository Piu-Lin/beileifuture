            ...
            {this.state.dangFei.map((item, i) => {
              return (
                <div
                  ...
                  onClick={() => {
                    this.changeDetail();
                    this.SetCard(item.name, item.seven, item.eight);
                  }}
                >
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    ...
                    this.SetCard(
                      item.name,
                      item.perPeopleProtectedMoney,
                       "人均保障金："+item.protectedPeople+"保障金："+item.houseProtectedMoney
                    );
                  }}
                >
                  <div className="TrItemTitle">{item.name}</div>
                  <div className="TrItemBottomLine">
                    <div className="TrItemTime">{item.perPeopleProtectedMoney}</div>
                    <div className="TrItemLookDetail">查看详情</div>
                  </div>
                </div>
              );
            ...
            {this.state.shouzhi.map((item, i) => {
              return (
                <div
                  ...
                  onClick={() => {
                    this.changeDetail();
                    this.SetCard(item.name, item.time, item.abstractInfo);
                  }}
                >
                  <div className="TrItemTitle">{item.name}</div>
                  <div className="TrItemBottomLine">
        ...
        <div className="index">
          <div className="TopNav">
            <div
              className="back"
              onClick={() =>
                isDetail ? this.changeDetail() : this.BackToHomeNav()
              }
            >
          ...
          {isDetail ? (
            <ThreeAffairsOpenDetail card={this.state.card} />
          ) : (
            <>
              <div className="TrSubNav">
                {this.state.TreNavState === 1 ? (
                  <div className="TrSubNavActive">党务公开</div>
                ) : (
                  <div
                    onClick={() => {
                      this.changeTreNavState(1);
                    }}
                  >
                    党务公开
                  </div>
                )}
                {this.state.TreNavState === 2 ? (
                  <div className="TrSubNavActive">村务公开</div>
                ) : (
                  <div
                    onClick={() => {
                      this.changeTreNavState(2);
                    }}
                  >
                    村务公开
                  </div>
                )}
                {this.state.TreNavState === 3 ? (
                  <div className="TrSubNavActive">财务公开</div>
                ) : (
                  <div
                    onClick={() => {
                      this.changeTreNavState(3);
                    }}
                  >
                    财务公开
                  </div>
